...
export default function getArrivalManualFlightSchedule(dateRange: {from: Date, to: Date}) {
    const startDate = dateRange.from.toDateString();
    const endDate = dateRange.to.toDateString();

    return useQuery({
        queryKey: ["arrival_manual_flight_schedule"],
        queryFn: async () => {
...
export async function submitArrivalManualFlightSchedule(datas) {
    const supabase = createClient();
...
export async function deleteArrivalManualFlightSchedule(datas) {
    const supabase = createClient();
    const { data } = await supabase.auth.getSession();
